...
// let OUTPUT_ROOT_ARG = args[1] ? path.resolve(process.cwd(), args[1]) : null; to -o "stats"
// let STORE_DIR_ARG = args[2] ? path.resolve(process.cwd(), args[2]) : null; to -a "assets"
let OUTPUT_ROOT_ARG = args.includes("-o")
  ? path.resolve(process.cwd(), args[args.indexOf("-o") + 1])
  : null;
let STORE_DIR_ARG = args.includes("-a")
  ? path.resolve(process.cwd(), args[args.indexOf("-a") + 1])
  : null;
const HIDE_LOG = args.includes("-hide-log");
...
async function processJsonFile(filePath, urlMap, contentMap, queue, paths) {
  ...
  if (found.length > 0) {
    console.log(
      `Found ${found.length} URL-like strings in ${path.basename(filePath)}`
    );
  }
